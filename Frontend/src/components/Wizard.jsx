...
export default function Wizard({ steps, onComplete, submitLabel = 'Finish' }) {
  ...
  const canNext = !steps[current].validate || steps[current].validate(data)

  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8"><Stepper steps={steps} currentStep={current} /></div>
      <div className="card dark:bg-gray-800 dark:border-gray-700 p-6 sm:p-8">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{steps[current].title}</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{steps[current].subtitle}</p>
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t">
          <button onClick={back} disabled={current === 0} className="btn-secondary flex items-center gap-2 disabled:opacity-40">
            <FiArrowLeft /> Back
          </button>
