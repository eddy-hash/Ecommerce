...
export default function Orders() {
  ...
  const load = useCallback(() => {
    ...
    orderApi.mine()
      ...
      .catch((err) => {
        console.error('orderApi.mine failed:', err?.response?.status, err?.response?.data)
        toast.error(`Failed to load orders (${err?.response?.status ?? 'network'})`)
        setOrders([])
      })
          ...
          {orders.map((o, i) => (
            <motion.div key={o.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="card p-4 sm:p-6">
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusColors[o.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}>
                  {o.status || '—'}
                </span>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 space-y-1">
                {(o.items || []).length === 0 && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 py-1">No line items in this order.</p>
                )}
                {(o.items || []).map((it, idx) => (
                  <div key={it.id ?? idx} className="flex justify-between text-sm">
                    <span className="text-gray-700 dark:text-gray-300">
                      {it.productName || `Product #${it.productId ?? '?'}`} × {it.quantity ?? 0}
                    </span>
                    <span className="font-medium text-gray-900 dark:text-white">
                      {format(it.priceAtPurchase ?? 0)}
                    </span>
                  </div>
                ))}
                <div className="flex justify-between pt-3 border-t border-gray-200 dark:border-gray-700 font-bold text-lg">
                  ...
                  <span className="text-brand-600">{format(o.totalAmount ?? 0)}</span>
                </div>
              </div>
            </motion.div>
          ))}
